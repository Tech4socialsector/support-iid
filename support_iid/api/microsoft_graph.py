...
# # Assumes GRAPH_URL and get_access_token() are already defined elsewhere
# # in this module (as in your original file). Not redefined here.
...
#     if manager_response.status_code != 200:
#         return {"exists": False, "message": "No manager found for this user."}

#     manager = manager_response.json()
#     manager_email = manager.get("mail") or manager.get("userPrincipalName")

#     if not manager_email:
#         return {"exists": False, "message": "Manager has no email on record."}
...
#         return {"exists": False, "message": "This is the top of the reporting chain — no skip-level manager."}
...
# Encryption
# --------------------------------------------------
# Must be identical (byte-for-byte, same base64 string) to the key used
# in the web form's Client Script decrypt function.
#
# Better practice than hardcoding: store this in site_config.json as
# "employee_lookup_aes_key" and load it with frappe.conf.get(...).
# Hardcoded here to match the simplest version of the fix.
